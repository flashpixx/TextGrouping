...
build.neighbourhood <- function( inputfile, stopwordlanguage = "english", progress = NA )
{
    # read csv (first column name, second column keywords)
    if ( !is.na( progress ) ) progress$set(message = "Read CSV", value = 0.2)
    ...
    # clean-up text data
    if ( !is.na( progress ) ) progress$set(message = "Clean-up text data", value = 0.2)
    ...
    if ( !is.na( progress ) ) progress$set(message = "Calculate normalized compression distances", value = 0.4)
    ...
    if ( !is.na( progress ) ) progress$set(message = "Multidimensional scaling", value = 0.6)
    ...
    if ( !is.na( progress ) ) progress$set(message = "Grouping with affinity propagation", value = 0.2)
    return(
        structure(
            class = "neighbourhood",
            list(
